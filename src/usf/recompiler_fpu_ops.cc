...
void ChangeDefaultRoundingModel(void)
{
    switch ((FPCR[31] & 3)) {
    case 0:
	FPU_RoundingMode = 0x0000;
	break;			//_RC_NEAR
    case 1:
	FPU_RoundingMode = 0x0C00;
	break;			//_RC_CHOP
    case 2:
	FPU_RoundingMode = 0x0800;
	break;			//_RC_UP
    case 3:
	FPU_RoundingMode = 0x0400;
	break;			//_RC_UP
    }
}
...
void CompileCop1Test(BLOCK_SECTION * Section)
{
    ...
    CompileExit(Section->CompilePC, &Section->RegWorking, COP1_Unuseable,
		0, (void (*) (uintptr_t))  JeLabel32);
    FpuBeenUsed = 1;
}
...
void Compile_R4300i_LWC1(BLOCK_SECTION * Section)
{
    ...
    if ((Opcode.ft & 1) != 0) {
	if (RegInStack(Section, Opcode.ft - 1, FPU_Double)
	    || RegInStack(Section, Opcode.ft - 1, FPU_Qword)) {
	    UnMap_FPR(Section, Opcode.ft - 1, 1);
	}
    }
    if (RegInStack(Section, Opcode.ft, FPU_Double)
	|| RegInStack(Section, Opcode.ft, FPU_Qword)) {
...
void Compile_R4300i_LDC1(BLOCK_SECTION * Section)
{
    ...
    MoveX86regPointerToX86reg(TempReg1, TempReg2, TempReg3);
    Push(TempReg2);
    MoveVariableToX86reg(&FPRDoubleLocation[Opcode.ft],
			 TempReg2 | x64_Reg);
    AddConstToX86Reg(TempReg2 | x64_Reg, 4);
    MoveX86regToX86Pointer(TempReg3, TempReg2);
    Pop(TempReg2);
...
void Compile_R4300i_SDC1(BLOCK_SECTION * Section)
{
	...
	MovePointerToX86reg((uint8_t *) & FPRDoubleLocation[Opcode.ft],
			    TempReg1);
    ...
    MoveVariableToX86reg((uint8_t *) & FPRDoubleLocation[Opcode.ft],
			 TempReg3 | x64_Reg);
...
void Compile_R4300i_COP1_MF(BLOCK_SECTION * Section)
{
    ...
    MovePointerToX86reg((uint8_t *) & FPRFloatLocation[Opcode.fs],
			TempReg);
...
void Compile_R4300i_COP1_DMF(BLOCK_SECTION * Section)
{
    ...
    MovePointerToX86reg((uint8_t *) & FPRDoubleLocation[Opcode.fs],
			TempReg);
...
void Compile_R4300i_COP1_MT(BLOCK_SECTION * Section)
{
    ...
    if ((Opcode.fs & 1) != 0) {
	if (RegInStack(Section, Opcode.fs - 1, FPU_Double)
	    || RegInStack(Section, Opcode.fs - 1, FPU_Qword)) {
	...
	MoveX86regToX86Pointer(Map_TempReg(Section, x86_Any, Opcode.rt, 0),
			       TempReg);
    }
}
...
void Compile_R4300i_COP1_DMT(BLOCK_SECTION * Section)
{
    ...
    if ((Opcode.fs & 1) == 0) {
	if (RegInStack(Section, Opcode.fs + 1, FPU_Float)
	    || RegInStack(Section, Opcode.fs + 1, FPU_Dword)) {
	    UnMap_FPR(Section, Opcode.fs + 1, 1);
	}
    }
    UnMap_FPR(Section, Opcode.fs, 1);
	...
	if Is64Bit
	    ...
	    MoveX86regToX86Pointer(Map_TempReg
	...
	AddConstToX86Reg64(TempReg, 4);
	MoveX86regToX86Pointer(Map_TempReg(Section, x86Reg, Opcode.rt, 1),
			       TempReg);
    }
}
...
void Compile_R4300i_COP1_CT(BLOCK_SECTION * Section)
{
	...
	MoveX86regToVariable(Map_TempReg(Section, x86_Any, Opcode.rt, 0),
			     &FPCR[Opcode.fs]);
    }
    Pushad();
...
void Compile_R4300i_COP1_S_SUB(BLOCK_SECTION * Section)
{
	...
	MovePointerToX86reg((uint8_t *) & FPRFloatLocation[Opcode.ft],
			    TempReg);
	...
	if (RegInStack(Section, Reg2, FPU_Float)) {
	    ...
	    MovePointerToX86reg((uint8_t *) & FPRFloatLocation[Reg2],
				TempReg);
	    fpuSubDwordRegPointer(TempReg);
	}
    }
...
void Compile_R4300i_COP1_S_TRUNC_L(BLOCK_SECTION * Section)
{
    ...
    CompileCop1Test(Section);
    if (Opcode.fd != Opcode.fs
	|| !RegInStack(Section, Opcode.fd, FPU_Float)) {
	Load_FPR_ToTop(Section, Opcode.fd, Opcode.fs, FPU_Float);
    }
    ChangeFPURegFormat(Section, Opcode.fd, FPU_Float, FPU_Qword,
		       RoundTruncate);
}

void Compile_R4300i_COP1_S_CEIL_L(BLOCK_SECTION * Section)
{				//added by Witten
...
void Compile_R4300i_COP1_S_FLOOR_L(BLOCK_SECTION * Section)
{				//added by Witten
...
void Compile_R4300i_COP1_S_ROUND_W(BLOCK_SECTION * Section)
{
    ...
    ChangeFPURegFormat(Section, Opcode.fd, FPU_Float, FPU_Dword,
		       RoundNearest);
}
...
void Compile_R4300i_COP1_S_CEIL_W(BLOCK_SECTION * Section)
{				// added by Witten
...
void Compile_R4300i_COP1_S_CVT_D(BLOCK_SECTION * Section)
{
    ...
    ChangeFPURegFormat(Section, Opcode.fd, FPU_Float, FPU_Double,
		       RoundDefault);
}
...
{
    uint32_t Reg1 =
	RegInStack(Section, Opcode.ft, FPU_Float) ? Opcode.ft : Opcode.fs;
    uint32_t Reg2 =
	RegInStack(Section, Opcode.ft, FPU_Float) ? Opcode.fs : Opcode.ft;
    int x86reg, cmp = 0;
...
void Compile_R4300i_COP1_D_ADD(BLOCK_SECTION * Section)
{
	...
	MovePointerToX86reg((uint8_t *) & FPRDoubleLocation[Reg2],
			    TempReg);
...
void Compile_R4300i_COP1_D_TRUNC_L(BLOCK_SECTION * Section)
{				//added by Witten


    CompileCop1Test(Section);
    if (RegInStack(Section, Opcode.fs, FPU_Double)
	|| RegInStack(Section, Opcode.fs, FPU_Qword)) {
	UnMap_FPR(Section, Opcode.fs, 1);
    }
    if (Opcode.fd != Opcode.fs
	|| !RegInStack(Section, Opcode.fd, FPU_Double)) {
	Load_FPR_ToTop(Section, Opcode.fd, Opcode.fs, FPU_Double);
    }
    ChangeFPURegFormat(Section, Opcode.fd, FPU_Double, FPU_Qword,
		       RoundTruncate);
}

void Compile_R4300i_COP1_D_CEIL_L(BLOCK_SECTION * Section)
{				//added by Witten
...
void Compile_R4300i_COP1_D_FLOOR_L(BLOCK_SECTION * Section)
{				//added by Witten
...
void Compile_R4300i_COP1_D_ROUND_W(BLOCK_SECTION * Section)
{
    ...
    ChangeFPURegFormat(Section, Opcode.fd, FPU_Double, FPU_Dword,
		       RoundNearest);
}
...
void Compile_R4300i_COP1_D_CEIL_W(BLOCK_SECTION * Section)
{				// added by Witten
...
void Compile_R4300i_COP1_D_FLOOR_W(BLOCK_SECTION * Section)
{				//added by Witten
...
void Compile_R4300i_COP1_D_CVT_S(BLOCK_SECTION * Section)
{
    ...
    ChangeFPURegFormat(Section, Opcode.fd, FPU_Double, FPU_Float,
		       RoundDefault);
}
...
void Compile_R4300i_COP1_W_CVT_S(BLOCK_SECTION * Section)
{
    ...
    CompileCop1Test(Section);
    if (Opcode.fd != Opcode.fs
	|| !RegInStack(Section, Opcode.fd, FPU_Dword)) {
	Load_FPR_ToTop(Section, Opcode.fd, Opcode.fs, FPU_Dword);
    }
    ChangeFPURegFormat(Section, Opcode.fd, FPU_Dword, FPU_Float,
		       RoundDefault);
}
...
void Compile_R4300i_COP1_W_CVT_D(BLOCK_SECTION * Section)
{
    ...
    ChangeFPURegFormat(Section, Opcode.fd, FPU_Dword, FPU_Double,
		       RoundDefault);
}
...
void Compile_R4300i_COP1_L_CVT_S(BLOCK_SECTION * Section)
{
    ...
    CompileCop1Test(Section);
    if (Opcode.fd != Opcode.fs
	|| !RegInStack(Section, Opcode.fd, FPU_Qword)) {
	Load_FPR_ToTop(Section, Opcode.fd, Opcode.fs, FPU_Qword);
    }
    ChangeFPURegFormat(Section, Opcode.fd, FPU_Qword, FPU_Float,
		       RoundDefault);
}
...
void Compile_R4300i_COP1_L_CVT_D(BLOCK_SECTION * Section)
{
    ...
    ChangeFPURegFormat(Section, Opcode.fd, FPU_Qword, FPU_Double,
		       RoundDefault);
}
