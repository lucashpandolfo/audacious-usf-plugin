...
void AndVariableDispToX86Reg(void *Variable, int32_t x86reg,
			     int32_t AddrReg, int32_t Multiplier)
{
#ifdef USEX64
    if (((uintptr_t) Variable - (uintptr_t) TLB_Map) < 0x7FFFFFFF) {
	OPCODE_REG_BASE_INDEX_SCALE_IMM32(8, 0x23, x86reg, x86_R15,
					  AddrReg, Indexes[Multiplier],
					  (uintptr_t) Variable -
					  (uintptr_t) TLB_Map);
    } else {
	LOAD_VARIABLE(x86_TEMP, Variable);
	OPCODE_REG_BASE_INDEX_SCALE(8, 0x23, x86reg, x86_TEMP, AddrReg,
				    Indexes[Multiplier]);
    }
#else
    OPCODE_REG_INDEX_SCALE_IMM32(8, 0x23, x86reg, AddrReg,
				 Indexes[Multiplier], Variable);
#endif
}
...
{
    OPCODE_REG_BASE_INDEX_SCALE(8, 0x8D, x86RegDest, x86_EBP, x86RegSrc,
				Indexes[multiplier]);
    PUTDST32(RecompPos, 0x00000000);
}

void LeaSourceAndOffset(int32_t x86DestReg, int32_t x86SourceReg,
			int32_t offset)
{
...
{
    OPCODE_REG_VARIABLE(PUTDST8(RecompPos, 0x66), 8, 0xC7, OP_D0,
			Variable);
    PUTDST16(RecompPos, Const);
}

void MoveConstHalfToX86regPointer(uint16_t Const, int32_t AddrReg1,
				  int32_t AddrReg2)
{
...
void MoveConstByteToX86regPointer(uint8_t Const, int32_t AddrReg1,
				  int32_t AddrReg2)
{
...
void MoveConstToX86regPointer(uint32_t Const, int32_t AddrReg1,
			      int32_t AddrReg2)
{
    OPCODE_REG_BASE_INDEX(8, 0xC7, OP_D0, AddrReg1, AddrReg2);
    PUTDST32(RecompPos, Const);
...
void MoveSxByteX86regPointerToX86reg(int32_t AddrReg1, int32_t AddrReg2,
				     int32_t x86reg)
{
    OPCODE_REG_BASE_INDEX(16, 0xBE0F, x86reg, AddrReg1, AddrReg2);
}

void MoveSxHalfX86regPointerToX86reg(int32_t AddrReg1, int32_t AddrReg2,
				     int32_t x86reg)
{
...
void MoveX86RegDispToX86Reg(int32_t x86reg, int32_t AddrReg,
			    int32_t IndexReg, int32_t Multiplier)
{
    OPCODE_REG_BASE_INDEX_SCALE(8, 0x8B, x86reg, AddrReg, IndexReg,
				Indexes[Multiplier]);
}

void MoveVariableDispToX86Reg(void *Variable, int32_t x86reg,
			      int32_t AddrReg, int32_t Multiplier)
{
#ifdef USEX64
    if (((uintptr_t) Variable - (uintptr_t) TLB_Map) < 0x7FFFFFFF) {
	OPCODE_REG_BASE_INDEX_SCALE_IMM32(8, 0x8B, x86reg, x86_R15,
					  AddrReg, Indexes[Multiplier],
					  ((uintptr_t) Variable -
					   (uintptr_t) TLB_Map));
    } else {
	LOAD_VARIABLE(x86_TEMP, Variable);
	OPCODE_REG_BASE_INDEX_SCALE(8, 0x8B, x86reg, x86_TEMP, AddrReg,
				    Indexes[Multiplier]);
    }
#else
    OPCODE_REG_INDEX_SCALE_IMM32(8, 0x8B, x86reg, AddrReg,
				 Indexes[Multiplier], Variable);
#endif
}
...
{
    BreakPoint();
    OPCODE_REG_VARIABLE(PUTDST8(RecompPos, 0x66), 8, 0x8B, x86reg,
			Variable);
}
...
void MoveX86regByteToX86regPointer(int32_t x86reg, int32_t AddrReg1,
				   int32_t AddrReg2)
{
...
{
    OPCODE_REG_VARIABLE(PUTDST8(RecompPos, 0x66), 8, 0x89, x86reg,
			Variable);
}

void MoveX86regHalfToX86regPointer(int32_t x86reg, int32_t AddrReg1,
				   int32_t AddrReg2)
{
    PUTDST8(RecompPos, 0x66);
...
void MoveX86regPointerToX86reg(int32_t AddrReg1, int32_t AddrReg2,
			       int32_t x86reg)
{
    OPCODE_REG_BASE_INDEX(8, 0x8B, x86reg, AddrReg1, AddrReg2);
}

void MoveX86regPointerToX86regDisp8(int32_t AddrReg1, int32_t AddrReg2,
				    int32_t x86reg, uint8_t offset)
{
    OPCODE_REG_BASE_INDEX_IMM8(8, 0x8B, x86reg, AddrReg1, AddrReg2,
			       offset);
}
...
void MoveX86regToX86regPointer(int32_t x86reg, int32_t AddrReg1,
			       int32_t AddrReg2)
{
    OPCODE_REG_BASE_INDEX(8, 0x89, x86reg, AddrReg1, AddrReg2);
}

void MoveZxByteX86regPointerToX86reg(int32_t AddrReg1, int32_t AddrReg2,
				     int32_t x86reg)
{
    OPCODE_REG_BASE_INDEX(16, 0xB60F, x86reg, AddrReg1, AddrReg2);
}

void MoveZxHalfX86regPointerToX86reg(int32_t AddrReg1, int32_t AddrReg2,
				     int32_t x86reg)
{
...
void Push(int32_t x86reg)
{
	...
	break;
    case x86_ESP:
	PUTDST8(RecompPos, 0x54);
	break;
    case x86_EBP:
	PUTDST8(RecompPos, 0x55);
	break;
    }
}
...
void Pop(int32_t x86reg)
{
	...
	break;
    case x86_ESP:
	PUTDST8(RecompPos, 0x5C);
	break;
    case x86_EBP:
	PUTDST8(RecompPos, 0x5D);
	break;
    }
}
...
void ShiftLeftDoubleImmed(int32_t Destination, int32_t Source,
			  uint8_t Immediate)
{
...
void ShiftRightDoubleImmed(int32_t Destination, int32_t Source,
			   uint8_t Immediate)
{
