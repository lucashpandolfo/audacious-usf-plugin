...
void RSPAndVariableDispToX86Reg(void *Variable, int32_t x86reg,
...
void RSPLeaRegReg(int32_t x86RegDest, int32_t x86RegSrc,
		  int32_t multiplier)
{
...
{
    OPCODE_REG_VARIABLE(PUTDST8(RSPRecompPos, 0x66), 8, 0xC7, OP_D0,
			Variable)
	PUTDST16(RSPRecompPos, Const);
}

void RSPMoveConstHalfToX86regPointer(uint16_t Const, int32_t AddrReg1,
				     int32_t AddrReg2)
{
...
{
    OPCODE_REG_ADDR_DMEM(PUTDST8(RSPRecompPos, 0x66), 8, 0x89, x86reg,
			 AddrReg);
}
...
void RSPMoveConstByteToX86regPointer(uint8_t Const, int32_t AddrReg1,
				     int32_t AddrReg2)
{
...
void RSPMoveConstToX86regPointer(uint32_t Const, int32_t AddrReg1,
				 int32_t AddrReg2)
{
    OPCODE_REG_BASE_INDEX(8, 0xC7, OP_D0, AddrReg1, AddrReg2);
    PUTDST32(RSPRecompPos, Const);
...
void RSPMoveN64MemToX86reg(int32_t x86reg, int32_t AddrReg)
{
	...
	OPCODE_REG_BASE_INDEX_IMM32(8, 0x8B, x86reg, x86_R15, AddrReg,
				    (uintptr_t) DMEM -
				    (uintptr_t) TLB_Map);
    } else {
...
void RSPMoveSxByteX86regPointerToX86reg(int32_t AddrReg1, int32_t AddrReg2,
					int32_t x86reg)
{
    OPCODE_REG_BASE_INDEX(16, 0xBE0F, x86reg, AddrReg1, AddrReg2);
}

void RSPMoveSxHalfX86regPointerToX86reg(int32_t AddrReg1, int32_t AddrReg2,
					int32_t x86reg)
{
...
void RSPMoveX86RegDispToX86Reg(int32_t x86reg, int32_t AddrReg,
			       ...
			       uint8_t Disp)
{
	...
	OPCODE_REG_BASE_INDEX_SCALE_IMM32(8, 0x8B, x86reg, x86_R15,
					  AddrReg, Indexes[0],
					  (((uintptr_t) DMEM -
					    (uintptr_t) TLB_Map)) + Disp);
    } else {
	LOAD_VARIABLE(x86_TEMP, DMEM);
	OPCODE_REG_BASE_INDEX_SCALE_IMM8(8, 0x8B, x86reg, x86_TEMP,
					 AddrReg, Indexes[0], Disp);
    }
#else
...
void RSPMoveVariableDispToX86Reg(void *Variable, int32_t x86reg,
				 int32_t AddrReg, int32_t Multiplier)
{
#ifdef USEX64
    if (((uintptr_t) Variable - (uintptr_t) TLB_Map) < 0x7FFFFFFF) {
...
{
    OPCODE_REG_VARIABLE(PUTDST8(RSPRecompPos, 0x66), 8, 0x8B, x86reg,
			Variable);
}
...
{
    OPCODE_REG_VARIABLE(PUTDST8(RSPRecompPos, 0x66), 8, 0x89, x86reg,
			Variable);
}
...
void RSPMoveX86regPointerToX86regDisp8(int32_t AddrReg1, int32_t AddrReg2,
...
void RSPMoveZxByteX86regPointerToX86reg(int32_t AddrReg1, int32_t AddrReg2,
					int32_t x86reg)
{
    OPCODE_REG_BASE_INDEX(16, 0xB60F, x86reg, AddrReg1, AddrReg2);
}

void RSPMoveZxHalfX86regPointerToX86reg(int32_t AddrReg1, int32_t AddrReg2,
					int32_t x86reg)
{
...
#ifdef USEX64
    if (((uintptr_t) DMEM - (uintptr_t) TLB_Map) < 0x7FFFFFFF) {
	OPCODE_REG_BASE_INDEX_IMM32(8, 0x89, x86reg, x86_R15, AddrReg,
				    ((uintptr_t) DMEM -
				     (uintptr_t) TLB_Map) + Disp);
    } else {
	LOAD_VARIABLE(x86_TEMP, DMEM);
...
{
    OPCODE_REG_VARIABLE(PUTDST8(RSPRecompPos, 0x66), 8, 0xb, x86Reg,
			Variable);
}

void RSPAndVariableToX86regHalf(void *Variable, int32_t x86Reg)
{
    OPCODE_REG_VARIABLE(PUTDST8(RSPRecompPos, 0x66), 8, 0x23, x86Reg,
			Variable);
}
