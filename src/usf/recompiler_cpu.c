...
void AddParent(BLOCK_SECTION * Section, BLOCK_SECTION * Parent)
{
	...
	Section->ParentSection =
	    malloc((NoOfParents + 1) * sizeof(void *));
    } else {
	Section->ParentSection =
...
/*	} __except( r4300i_CPU_MemoryFilter( GetExceptionCode(), GetExceptionInformation()) ) {
	...
	if (*TargetPC[count] != (uint32_t) - 1
	    ...
	    *TargetSection[count] =
		ExistingSection(BlockInfo.BlockInfo.ContinueSection,
				*TargetPC[count], GetNewTestValue());
	    if (*TargetSection[count] == NULL) {
		*TargetSection[count] =
		    ExistingSection(BlockInfo.BlockInfo.JumpSection,
				    *TargetPC[count], GetNewTestValue());
	    }
	    if (*TargetSection[count] == NULL) {
		BlockInfo.NoOfSections += 1;
		   ...
		   uint32_t TestID)
{
	...
	DetermineLoop(Section->ContinueSection, Test, GetNewTestValue(),
		      ((BLOCK_SECTION *) Section->ContinueSection)->
		      SectionID);
    }
    if (Section->JumpSection != NULL) {
	DetermineLoop(Section->JumpSection, Test, GetNewTestValue(),
...
void FixConstants(BLOCK_SECTION * Section, uint32_t Test, int *Changed)
{
	...
	*Changed = 1;
	...
	FixConstants(Section->JumpSection, Test, Changed);
    }
    if (Section->ContinueSection) {
	FixConstants(Section->ContinueSection, Test, Changed);
    }
}
...
void FreeSection(BLOCK_SECTION * Section, BLOCK_SECTION * Parent)
{
	...
	if (Section->ParentSection) {
	    for (count = 0; count < NoOfParents; count++) {
		if (!IsAllParentLoops
		    (Section, Section->ParentSection[count], 0,
		     GetNewTestValue())) {
		    return;
		}
	    }
...
uint32_t GenerateX86Code(BLOCK_SECTION * Section, uint32_t Test)
{
	...
	Section->Test = Test;
	if (GenerateX86Code(Section->ContinueSection, Test)) {
	    return 1;
	}
	if (GenerateX86Code(Section->JumpSection, Test)) {
	    return 1;
	}
	return 0;
    }
			  ...
			  uint32_t IgnoreIfCompiled, uint32_t Test)
{
	...
	if (!IsAllParentLoops
	    (Section, Parent->ParentSection[count], IgnoreIfCompiled,
	     Test)) {
	    return 0;
	}
    }
