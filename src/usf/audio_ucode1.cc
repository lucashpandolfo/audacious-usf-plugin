...
{				// memcpy causes static... endianess issue :(
    ...
    memcpy(BufferSpace + (AudioInBuffer & 0xFFFC), (u8 *) RDRAM + v0,
	   (AudioCount + 3) & 0xFFFC);
...
{				// memcpy causes static... endianess issue :(
    ...
    memcpy((unsigned char *) RDRAM + v0,
	   BufferSpace + (AudioOutBuffer & 0xFFFC),
	   (AudioCount + 3) & 0xFFFC);
}
...
void (*ABI1[0x20]) () = {	// TOP Performace Hogs: MIXER, RESAMPLE, ENVMIXER
    ...
    UNKNOWN, SETBUFF, SETVOL, DMEMMOVE, LOADADPCM, MIXER,
    INTERLEAVE, UNKNOWN, SETLOOP, SPNOOP, SPNOOP, SPNOOP, SPNOOP,
