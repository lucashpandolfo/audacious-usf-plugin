...
void ReOrderInstructions(uint32_t StartPC, uint32_t EndPC)
{
    ...
    if (IsOpcodeNop(StartPC) && IsOpcodeNop(StartPC + 4)
	&& IsOpcodeNop(StartPC + 8)) {
	/* Dont even bother */
	return;
    }
	...
	for (;;) {
	    ...
	    if (1 ==
		CompareInstructions(CurrentPC, &PreviousOp, &CurrentOp)) {
		...
		*(uint32_t *) (IMEM + CurrentPC) = PreviousOp.Hex;
	    ...
	    if (IsOpcodeNop(CurrentPC) && IsOpcodeNop(CurrentPC + 4)
		&& IsOpcodeNop(CurrentPC + 8)) {
		CurrentPC = EndPC;
	    }
	}
...
void LinkBranches(RSP_BLOCK * Block)
{
    ...
    CPU_Message("***** Linking branches (%i) *****",
		RSPCurrentBlock.ResolveCount);
	...
	CPU_Message
	    ("Linked RSP branch from x86: %08X, to RSP: %X / x86: %08X",
...
#ifdef BUILD_BRANCHLABELS_VERBOSE
		CPU_Message("[%02i] Added branch at %X to %X",
			    RspCode.LabelCount, i, Dest);
#endif
	    } else {
...
{
    uint32_t Count, Padding, X86BaseAddress =
	(uint32_t) (intptr_t) RSPRecompPos;
    ...
    *(RSPJumpTable + (RSPCompilePC >> 2)) = RSPRecompPos;
	...
	if (RSPNextInstruction == NORMAL && IsJumpLabel(RSPCompilePC)) {
		...
		CPU_Message
		    ("***** Adding Jump Table Entry for PC: %04X at X86: %08X *****",
		     RSPCompilePC, RSPRecompPos);
		CPU_Message("");
		*(RSPJumpTable + (RSPCompilePC >> 2)) = RSPRecompPos;
	...
	case FINISH_SUB_BLOCK:
		...
		CPU_Message
		    ("**** Continuing static SubBlock (jump table entry added for PC: %04X at X86: %08X) *****",
		     RSPCompilePC, RSPRecompPos);
		*(RSPJumpTable + (RSPCompilePC >> 2)) = RSPRecompPos;
