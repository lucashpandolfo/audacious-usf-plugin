...
void RSP_Opcode_BEQ(void)
{
	...
	RSP_JumpTo =
	    (*PrgCount + ((int16_t) RSPOpC.offset << 2) + 4) & 0xFFC;
    } else {
...
void RSP_Opcode_ADDI(void)
{
    if (RSPOpC.rt != 0) {
	RSP_GPR[RSPOpC.rt].W =
	    RSP_GPR[RSPOpC.rs].W + (int16_t) RSPOpC.immediate;
    }
}
...
{
    if (RSPOpC.rt != 0) {
	RSP_GPR[RSPOpC.rt].UW =
	    RSP_GPR[RSPOpC.rs].UW +
	    (uint32_t) ((int16_t) RSPOpC.immediate);
    }
}
...
{
    uint32_t Address =
	((RSP_GPR[RSPOpC.base].UW + (int16_t) RSPOpC.offset) & 0xFFF);
...
void RSP_Special_SLLV(void)
{
    if (RSPOpC.rd != 0) {
	RSP_GPR[RSPOpC.rd].W =
	    RSP_GPR[RSPOpC.rt].W << (RSP_GPR[RSPOpC.rs].W & 0x1F);
    }
}
...
{
    if (RSPOpC.rd != 0) {
	RSP_GPR[RSPOpC.rd].UW =
	    RSP_GPR[RSPOpC.rt].UW >> (RSP_GPR[RSPOpC.rs].W & 0x1F);
    }
}
...
void RSP_Special_ADDU(void)
{
    if (RSPOpC.rd != 0) {
	RSP_GPR[RSPOpC.rd].UW =
	    RSP_GPR[RSPOpC.rs].UW + RSP_GPR[RSPOpC.rt].UW;
    }
}
...
void RSP_Cop0_MF(void)
{
    switch (RSPOpC.rd) {
    case 4:
	RSP_GPR[RSPOpC.rt].UW = SP_STATUS_REG;
	break;
    case 5:
	RSP_GPR[RSPOpC.rt].UW = SP_DMA_FULL_REG;
	break;
    case 6:
	RSP_GPR[RSPOpC.rt].UW = SP_DMA_BUSY_REG;
	break;
    case 7:
	RSP_GPR[RSPOpC.rt].W = 0;
	...
	break;
    case 8:
	RSP_GPR[RSPOpC.rt].UW = DPC_START_REG;
	break;
    case 9:
	RSP_GPR[RSPOpC.rt].UW = DPC_END_REG;
	break;
    case 10:
	RSP_GPR[RSPOpC.rt].UW = DPC_CURRENT_REG;
	break;
    case 11:
	RSP_GPR[RSPOpC.rt].W = DPC_STATUS_REG;
	break;
    case 12:
	RSP_GPR[RSPOpC.rt].W = DPC_CLOCK_REG;
	break;
    default:
...
void RSP_Cop0_MT(void)
{
    switch (RSPOpC.rd) {
    case 0:
	SP_MEM_ADDR_REG = RSP_GPR[RSPOpC.rt].UW;
	break;
    case 1:
	SP_DRAM_ADDR_REG = RSP_GPR[RSPOpC.rt].UW;
	break;
    case 2:
	...
	if ((RSP_GPR[RSPOpC.rt].W & DPC_CLR_PIPE_CTR) != 0) {
	    printf("RSP: DPC_STATUS_REG: DPC_CLR_PIPE_CTR");
	}
	if ((RSP_GPR[RSPOpC.rt].W & DPC_CLR_CMD_CTR) != 0) {
	    printf("RSP: DPC_STATUS_REG: DPC_CLR_CMD_CTR");
	}
...
void RSP_Cop2_MF(void)
{
    ...
    RSP_GPR[RSPOpC.rt].B[0] =
	RSP_Vect[RSPOpC.rd].B[15 - ((element + 1) % 16)];
    RSP_GPR[RSPOpC.rt].W = RSP_GPR[RSPOpC.rt].HW[0];
}
...
void RSP_Cop2_CF(void)
{
    ...
    case 0:
	RSP_GPR[RSPOpC.rt].W = RSP_Flags[0].HW[0];
	break;
    case 1:
	RSP_GPR[RSPOpC.rt].W = RSP_Flags[1].HW[0];
	break;
    case 2:
	RSP_GPR[RSPOpC.rt].W = RSP_Flags[2].HW[0];
	break;
    case 3:
	RSP_GPR[RSPOpC.rt].W = RSP_Flags[2].HW[0];
	break;
...
void RSP_Cop2_CT(void)
{
    switch ((RSPOpC.rd & 0x03)) {
    case 0:
	RSP_Flags[0].HW[0] = RSP_GPR[RSPOpC.rt].HW[0];
	break;
    case 1:
	RSP_Flags[1].HW[0] = RSP_GPR[RSPOpC.rt].HW[0];
	break;
    case 2:
	RSP_Flags[2].B[0] = RSP_GPR[RSPOpC.rt].B[0];
	break;
    case 3:
	RSP_Flags[2].B[0] = RSP_GPR[RSPOpC.rt].B[0];
	break;
...
void RSP_Vector_VMULF(void)
{
	...
	if (RSP_Vect[RSPOpC.rd].UHW[el] != 0x8000
	    || RSP_Vect[RSPOpC.rt].UHW[del] != 0x8000) {
	    temp.W =
		((int32_t) RSP_Vect[RSPOpC.rd].HW[el] *
		 (int32_t) RSP_Vect[RSPOpC.rt].HW[del]) << 1;
	    temp.UW += 0x8000;
...
void RSP_Vector_VMULU(void)
{
	...
	del = EleSpec[RSPOpC.rs].B[el];
	RSP_ACCUM[el].DW =
	    (int64_t) (RSP_Vect[RSPOpC.rd].HW[el] *
		       RSP_Vect[RSPOpC.rt].HW[del]) << 17;
	RSP_ACCUM[el].DW += 0x80000000;
	if (RSP_ACCUM[el].DW < 0) {
	    RSP_Vect[RSPOpC.sa].HW[el] = 0;
	} else if ((int16_t) (RSP_ACCUM[el].UHW[3] ^ RSP_ACCUM[el].UHW[2])
		   < 0) {
...
void RSP_Vector_VMUDL(void)
{
	...
	temp.UW =
	    (uint32_t) RSP_Vect[RSPOpC.rd].UHW[el] *
	    (uint32_t) RSP_Vect[RSPOpC.rt].UHW[del];
...
//      cprintf("%08x%08x x %08x%08x = ", RSP_Vect[RSPOpC.rd].DW[0], RSP_Vect[RSPOpC.rd].DW[1],RSP_Vect[RSPOpC.rt].DW[0],RSP_Vect[RSPOpC.rt].DW[1])
	...
	temp.UW =
	    (uint32_t) ((int32_t) RSP_Vect[RSPOpC.rd].HW[el]) *
	    (uint32_t) RSP_Vect[RSPOpC.rt].UHW[del];
	if (temp.W < 0) {
	    RSP_ACCUM[el].HW[3] = -1;
	} else {
...
void RSP_Vector_VMUDN(void)
{
	...
	temp.UW =
	    (uint32_t) RSP_Vect[RSPOpC.rd].UHW[el] *
	    (uint32_t) (int32_t) (RSP_Vect[RSPOpC.rt].HW[del]);
	if (temp.W < 0) {
	    RSP_ACCUM[el].HW[3] = -1;
	} else {
	    RSP_ACCUM[el].HW[3] = 0;
...
void RSP_Vector_VMUDH(void)
{
	...
	RSP_ACCUM[el].W[1] =
	    (int32_t) RSP_Vect[RSPOpC.rd].HW[el] *
	    (int32_t) RSP_Vect[RSPOpC.rt].HW[del];
	RSP_ACCUM[el].HW[1] = 0;
...
void RSP_Vector_VMACF(void)
{
	...
	temp.W =
	    (int32_t) RSP_Vect[RSPOpC.rd].HW[el] *
	    (int32_t) (uint32_t) RSP_Vect[RSPOpC.rt].HW[del];
...
void RSP_Vector_VABS(void)
{
	...
	if (RSP_Vect[RSPOpC.rd].HW[el] > 0) {
	    ...
	    if (RSP_Vect[RSPOpC.rt].UHW[EleSpec[RSPOpC.rs].B[el]] ==
		0x8000) {
		RSP_Vect[RSPOpC.sa].HW[el] = 0x7FFF;
	    } else {
		RSP_Vect[RSPOpC.sa].HW[el] =
		    RSP_Vect[RSPOpC.rt].HW[del] * -1;
	    }
	} else {
	    RSP_Vect[RSPOpC.sa].HW[el] = 0;
	}
...
void RSP_Vector_VADDC(void)
{
	...
	temp.UW =
	    (int32_t) RSP_Vect[RSPOpC.rd].UHW[el] +
	    (int32_t) RSP_Vect[RSPOpC.rt].UHW[del];
	RSP_ACCUM[el].HW[1] = temp.HW[0];
	RSP_Vect[RSPOpC.sa].HW[el] = temp.HW[0];
...
void RSP_Vector_VLT(void)
{
	...
	} else if (RSP_Vect[RSPOpC.rd].HW[el] !=
		   RSP_Vect[RSPOpC.rt].HW[del]) {
	    RSP_Vect[RSPOpC.sa].HW[el] = RSP_Vect[RSPOpC.rt].UHW[del];
	    RSP_Flags[1].UW &= ~(1 << (7 - el));
	} else {
	    RSP_Vect[RSPOpC.sa].HW[el] = RSP_Vect[RSPOpC.rd].UHW[el];
	    if ((RSP_Flags[0].UW & (0x101 << (7 - el))) ==
		(uint16_t) (0x101 << (7 - el))) {
		RSP_Flags[1].UW |= (1 << (7 - el));
	    } else {
		RSP_Flags[1].UW &= ~(1 << (7 - el));
...
void RSP_Vector_VCL(void)
{
	...
	if ((RSP_Flags[0].UW & (1 << (7 - el))) != 0) {
		...
		if ((RSP_Flags[2].UW & (1 << (7 - el)))) {
		    if (RSP_Vect[RSPOpC.rd].UHW[el] +
			RSP_Vect[RSPOpC.rt].UHW[del] > 0x10000) {
			RSP_ACCUM[el].HW[1] = RSP_Vect[RSPOpC.rd].HW[el];
			RSP_Flags[1].UW &= ~(1 << (7 - el));
		    } else {
			RSP_ACCUM[el].HW[1] =
			    -RSP_Vect[RSPOpC.rt].UHW[del];
			RSP_Flags[1].UW |= (1 << (7 - el));
		    }
		} else {
		    if (RSP_Vect[RSPOpC.rt].UHW[del] +
			RSP_Vect[RSPOpC.rd].UHW[el] != 0) {
			RSP_ACCUM[el].HW[1] = RSP_Vect[RSPOpC.rd].HW[el];
			RSP_Flags[1].UW &= ~(1 << (7 - el));
		    } else {
			RSP_ACCUM[el].HW[1] =
...
void RSP_Vector_VAND(void)
{
	...
	del = EleSpec[RSPOpC.rs].B[el];
	RSP_Vect[RSPOpC.sa].HW[el] =
	    RSP_Vect[RSPOpC.rd].HW[el] & RSP_Vect[RSPOpC.rt].HW[del];
	RSP_ACCUM[el].HW[1] = RSP_Vect[RSPOpC.sa].HW[el];
    }
}
...
void RSP_Vector_VRCP(void)
{
    ...
    RecpResult.W =
	RSP_Vect[RSPOpC.rt].HW[EleSpec[RSPOpC.rs].B[(RSPOpC.rd & 0x7)]];
    if (RecpResult.UW == 0) {
	...
	RSP_ACCUM[count].HW[1] =
	    RSP_Vect[RSPOpC.rt].UHW[EleSpec[RSPOpC.rs].B[count]];
    }
...
void RSP_Vector_VRCPL(void)
{
    ...
    RecpResult.UW =
	RSP_Vect[RSPOpC.rt].UHW[EleSpec[RSPOpC.rs].
				B[(RSPOpC.rd & 0x7)]] | Recp.W;
    if (RecpResult.UW == 0) {
	RecpResult.UW = 0x7FFFFFFF;
    } else {
	if (RecpResult.W < 0) {
...
void RSP_Vector_VRSQ(void)
{
	...
	SQrootResult.W =
	    (int32_t) (0x7FFFFFFF /
		       (int32_t) sqrt((double) SQrootResult.W));
	for (count = 31; count > 0; count--) {
	    if ((SQrootResult.W & (1 << count))) {
...
{
    uint32_t Address =
	((RSP_GPR[RSPOpC.base].UW + (uint32_t) (RSPOpC.voffset)) & 0xFFF);
...
{
    uint32_t Address =
	((RSP_GPR[RSPOpC.base].UW +
	  (uint32_t) (RSPOpC.voffset << 1)) & 0xFFF);
...
void RSP_Opcode_LLV(void)
{
    uint32_t Address =
	((RSP_GPR[RSPOpC.base].UW +
	  (uint32_t) (RSPOpC.voffset << 2)) & 0xFFF);
...
void RSP_Opcode_LDV(void)
{
    uint32_t Address =
	((RSP_GPR[RSPOpC.base].UW +
	  (uint32_t) (RSPOpC.voffset << 3)) & 0xFFF);
...
void RSP_Opcode_LQV(void)
{
    uint32_t Address =
	((RSP_GPR[RSPOpC.base].UW +
	  (uint32_t) (RSPOpC.voffset << 4)) & 0xFFF);
