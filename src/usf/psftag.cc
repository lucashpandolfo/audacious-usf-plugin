		       ...
		       const char *variable, const char *value)
{
	...
	while (insert_i
	       && (((unsigned) (tag[insert_i - 1])) & 0xFF) <= 0x20) {
	    insert_i--;
	}
	/* Insert a newline if there's room and if there's stuff before */
	if (insert_i && (insert_i < tag_max_usable_size)) {
	    tag[insert_i++] = 0x0A;
	}
	/* Clamp insert length */
	if ((insert_i + insert_l) > tag_max_usable_size) {
	    insert_l = tag_max_usable_size - insert_i;
	}
	z = insert_i + insert_l;
	/*
	...
	movel = tag_l - i_end;
	if (movel > (tag_max_usable_size - (insert_i + insert_l))) {
	    movel = tag_max_usable_size - (insert_i + insert_l);
	}
	/* perform the move */
    ...
    if (z > tag_max_usable_size)
	z = tag_max_usable_size;
    tag[z] = 0;
    /*
	...
	if (insert_i >= insert_l)
	    break;
	tag[insert_i++] = '=';
	if (insert_i >= insert_l)
	    break;
	for (; (*value) && ((*value) != 0x0A) && (insert_i < insert_l);
	     value++) {
	    tag[insert_i++] = *value;
	}
	if (insert_i >= insert_l)
