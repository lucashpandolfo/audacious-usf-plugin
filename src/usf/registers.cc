...
uint32_t PROGRAM_COUNTER, *CP0, *FPCR, *RegRDRAM, *RegSP;
uint32_t *RegDPC, *RegMI, *RegVI, *RegAI, *RegPI,  *RegRI, *RegSI, HalfLine;
uint32_t RegModValue, ViFieldNumber, LLBit, LLAddr;
...
void SetupRegisters(N64_REGISTERS * n64_Registers)
{
    ...
    PIF_Ram = (uint8_t *) n64_Registers->PIF_Ram;
}
			...
			int32_t RoundingModel)
{
	...
	if (FpuMappedTo(i) == (uint32_t) Reg) {
		...
		ChangeFPURegFormat(Section, Reg, OldFormat, NewFormat,
				   RoundingModel);
		return;
	    }
...
void ChangeMiModeReg(void)
{
    ...
    if ((RegModValue & MI_CLR_INIT) != 0) {
	MI_MODE_REG &= ~MI_MODE_INIT;
    }
    if ((RegModValue & MI_SET_INIT) != 0) {
	MI_MODE_REG |= MI_MODE_INIT;
    }
    if ((RegModValue & MI_CLR_EBUS) != 0) {
	MI_MODE_REG &= ~MI_MODE_EBUS;
    }
    if ((RegModValue & MI_SET_EBUS) != 0) {
	MI_MODE_REG |= MI_MODE_EBUS;
    }
    if ((RegModValue & MI_CLR_DP_INTR) != 0) {
	MI_INTR_REG &= ~MI_INTR_DP;
    }
    if ((RegModValue & MI_CLR_RDRAM) != 0) {
	MI_MODE_REG &= ~MI_MODE_RDRAM;
    }
    if ((RegModValue & MI_SET_RDRAM) != 0) {
	MI_MODE_REG |= MI_MODE_RDRAM;
    }
}
...
int32_t Free8BitX86Reg(BLOCK_SECTION * Section)
{
    ...
    if (x86Mapped(x86_EBX & 0x2f) == NotMapped
	&& !x86Protected(x86_EBX & 0x2f)) {
	return x86_EBX;
    }
    if (x86Mapped(x86_EDX & 0x2f) == NotMapped
	&& !x86Protected(x86_EDX & 0x2f)) {
	return x86_EDX;
    }
    if (x86Mapped(x86_ECX & 0x2f) == NotMapped
	&& !x86Protected(x86_ECX & 0x2f)) {
	return x86_ECX;
    }
    if (x86Mapped(x86_EAX & 0x2f) == NotMapped
	&& !x86Protected(x86_EAX & 0x2f)) {
	return x86_EAX;
    }
#ifdef EXT_REGS
    if (x86Mapped(x86_R9D & 0x2f) == NotMapped
	&& !x86Protected(x86_R9D & 0x2f)) {
	return x86_R9D;
    }
    if (x86Mapped(x86_R10D & 0x2f) == NotMapped
	&& !x86Protected(x86_R10D & 0x2f)) {
	return x86_R10D;
    }
    if (x86Mapped(x86_R11D & 0x2f) == NotMapped
	&& !x86Protected(x86_R11D & 0x2f)) {
	return x86_R11D;
    }
    if (x86Mapped(x86_R14D & 0x2f) == NotMapped
	&& !x86Protected(x86_R14D & 0x2f)) {
	return x86_R14D;
    }
#endif
...
int32_t FreeX86Reg(BLOCK_SECTION * Section)
{
    ...
    if (x86Mapped(x86_ESI & 0x2f) == NotMapped
	&& !x86Protected(x86_ESI & 0x2f)) {
	return x86_ESI;
    }
    if (x86Mapped(x86_EDI & 0x2f) == NotMapped
	&& !x86Protected(x86_EDI & 0x2f)) {
	return x86_EDI;
    }
    if (x86Mapped(x86_EBX & 0x2f) == NotMapped
	&& !x86Protected(x86_EBX & 0x2f)) {
    ...
    if (x86Mapped(x86_R12D & 0x2f) == NotMapped
	&& !x86Protected(x86_R12D & 0x2f)) {
	return x86_R12D;
    }
    if (x86Mapped(x86_R13D & 0x2f) == NotMapped
	&& !x86Protected(x86_R13D & 0x2f)) {
	return x86_R13D;
    }
    if (x86Mapped(x86_R14D & 0x2f) == NotMapped
	&& !x86Protected(x86_R14D & 0x2f)) {
    ...
    for (count = 0; count < 64; count++) {
	if (MapCount[count] > 0
	    && x86Mapped(MapReg[count]) != Stack_Mapped) {
	    if (UnMap_X86reg(Section, MapReg[count])) {
		return MapReg[count];
	    }
	}
	if (x86Mapped(MapReg[count]) == Stack_Mapped) {
	    StackReg = MapReg[count];
	}
    }
    if (StackReg >= 0) {
		    ...
		    int32_t RegToLoad, int32_t Format)
{
	...
	if ((Reg & 1) != 0) {
		...
		if (FpuMappedTo(i) == (uint32_t) (Reg - 1)) {
		    if (FpuState(i) == FPU_Double
			|| FpuState(i) == FPU_Qword) {
			UnMap_FPR(Section, Reg, 1);
		    }
		    i = 8;
		}
	...
	if (Reg != RegToLoad) {
		...
		fpuLoadReg((int32_t *) & StackTopPos,
			   StackPosition(Section, RegToLoad));
	...
	case FPU_Dword:
	    ...
	    fpuLoadIntegerDwordFromX86Reg((int32_t *) & StackTopPos,
					  TempReg);
	    break;
	case FPU_Qword:
	    MovePointerToX86reg(&FPRDoubleLocation[RegToLoad], TempReg);
	    fpuLoadIntegerQwordFromX86Reg((int32_t *) & StackTopPos,
					  TempReg);
	    break;
	case FPU_Float:
	    MovePointerToX86reg(&FPRFloatLocation[RegToLoad], TempReg);
	    fpuLoadDwordFromX86Reg((int32_t *) & StackTopPos, TempReg);
	    break;
	case FPU_Double:
	    MovePointerToX86reg(&FPRDoubleLocation[RegToLoad], TempReg);
	    fpuLoadQwordFromX86Reg((int32_t *) & StackTopPos, TempReg);
	    break;
	}
		   ...
		   uint32_t SignValue, int32_t MipsRegToLoad)
{
    ...
    MipsRegState(Reg) =
	SignValue ? STATE_MAPPED_32_SIGN : STATE_MAPPED_32_ZERO;
}
		   ...
		   int32_t MipsRegToLoad)
{
	...
	if (IsUnknown(MipsRegToLoad)) {
	    ...
	    if (Is32Bit(MipsRegToLoad)) {
		if (IsSigned(MipsRegToLoad)) {
		    if (MipsRegLo((int) MipsRegLo(MipsRegToLoad) >> 31) !=
			0) {
			MoveConstToX86reg((int) MipsRegLo(MipsRegToLoad) >>
					  31, x86Hi);
		    } else {
			XorX86RegToX86Reg(x86Hi, x86Hi);
		    }
		    ...
		    int32_t MipsReg, uint32_t LoadHiWord)
{
	...
	if (x86Mapped(x86_EBX & 0x2f) == Temp_Mapped
	    && !x86Protected(x86_EBX & 0x2f)) {
	    x86Reg = x86_EBX;
	}
	if (x86Mapped(x86_EAX & 0x2f) == Temp_Mapped
	    && !x86Protected(x86_EAX & 0x2f)) {
	    x86Reg = x86_EAX;
	}
	if (x86Mapped(x86_EDX & 0x2f) == Temp_Mapped
	    && !x86Protected(x86_EDX & 0x2f)) {
	    x86Reg = x86_EDX;
	}
	if (x86Mapped(x86_ECX & 0x2f) == Temp_Mapped
	    && !x86Protected(x86_ECX & 0x2f)) {
	    x86Reg = x86_ECX;
	}
#ifdef EXT_REGS
	if (x86Mapped(x86_R9D & 0x2f) == Temp_Mapped
	    && !x86Protected(x86_R9D & 0x2f)) {
	    x86Reg = x86_R9D;
	}
	if (x86Mapped(x86_R10D & 0x2f) == Temp_Mapped
	    && !x86Protected(x86_R10D & 0x2f)) {
	    x86Reg = x86_R10D;
	}
	if (x86Mapped(x86_R11D & 0x2f) == Temp_Mapped
	    && !x86Protected(x86_R11D & 0x2f)) {
	    x86Reg = x86_R11D;
	}
	if (x86Mapped(x86_R14D & 0x2f) == Temp_Mapped
	    && !x86Protected(x86_R14D & 0x2f)) {
	    x86Reg = x86_R14D;
	}
#endif
	...
	if (x86Mapped(x86Reg) == GPR_Mapped) {
		...
		if (IsMapped(count)) {
			...
			if (MipsReg == count && LoadHiWord == 0) {
			    MipsReg = -1;
			}
			count = 32;
		    }
		    if (Is64Bit(count)
			&& MipsRegHi(count) == (uint32_t) x86Reg) {
			if (NewReg < 0) {
			    UnMap_GPR(Section, count, 1);
			    count = 32;
			    continue;
	...
	if (LoadHiWord) {
		...
		if (Is64Bit(MipsReg)) {
			...
			MoveConstToX86reg((int) MipsRegLo(MipsReg) >> 31,
					  x86Reg);
		    } else {
			XorX86RegToX86Reg(x86Reg, x86Reg);
		    }
...
void SetFpuLocations(void)
{
	...
	for (count = 0; count < 32; count++) {
	    FPRFloatLocation[count] =
		(void *) (&FPR[count >> 1].W[count & 1]);
	       ...
	       int32_t WriteBackValue)
{
	...
	if (WriteBackValue) {
		...
		case RoundDefault:
		    OrVariableToX86Reg(&FPU_RoundingMode, x86reg);
		    break;
		case RoundTruncate:
		    OrConstToX86Reg(0x0C00, x86reg);
		    break;
		case RoundNearest:	/*OrConstToX86Reg(0x0000, x86reg); */
		    break;
		case RoundDown:
		    OrConstToX86Reg(0x0400, x86reg);
		    break;
		case RoundUp:
		    OrConstToX86Reg(0x0800, x86reg);
		    break;
		}
	    ...
	    case FPU_Dword:
		MovePointerToX86reg(&FPRFloatLocation
				    [FpuMappedTo(StackTopPos)], TempReg);
		fpuStoreIntegerDwordFromX86Reg((int32_t *) & StackTopPos,
					       TempReg, 1);
		break;
	    case FPU_Qword:
		MovePointerToX86reg(&FPRDoubleLocation
				    [FpuMappedTo(StackTopPos)], TempReg);
		fpuStoreIntegerQwordFromX86Reg((int32_t *) & StackTopPos,
					       TempReg, 1);
		break;
	    case FPU_Float:
		MovePointerToX86reg(&FPRFloatLocation
				    [FpuMappedTo(StackTopPos)], TempReg);
		fpuStoreDwordFromX86Reg((int32_t *) & StackTopPos, TempReg,
					1);
		break;
	    case FPU_Double:
		MovePointerToX86reg(&FPRDoubleLocation
				    [FpuMappedTo(StackTopPos)], TempReg);
		fpuStoreQwordFromX86Reg((int32_t *) & StackTopPos, TempReg,
					1);
		break;
	    }
	    x86Protected(TempReg) = 0;
...
uint32_t UnMap_X86reg(BLOCK_SECTION * Section, uint32_t x86Reg)
{
    ...
    if (x86Mapped(x86Reg) == NotMapped && x86Protected(x86Reg) == 0) {
	return 1;
    }
...
void WriteBackRegisters(BLOCK_SECTION * Section)
{
	...
	case STATE_CONST_64:
	    ...
	    if (MipsRegLo(count) == 0xFFFFFFFF
		|| MipsRegHi(count) == 0xFFFFFFFF) {
		MoveConstToX86reg(0xFFFFFFFF, x86_ESI);
		bEsiSign = 1;
	    }
