...
{				// Needs accuracy verification...
    ...
    memcpyfn64((u8 *) (uintptr_t) BufferSpace + (inst1 & 0xfffc),
	       (u8 *) (uintptr_t) v0, (cnt + 3) & 0xFFFC);
}
...
{				// Needs accuracy verification...
    ...
    u32 cnt = (((inst1 >> 0xC) + 3) & 0xFFC);
    v0 = (inst2 & 0xfffffc);	// + SEGMENTS[(inst2>>24)&0xf];
    memcpy2n64((u8 *) (uintptr_t) v0,
	       (u8 *) BufferSpace + (inst1 & 0xfffc), (cnt + 3) & 0xFFFC);
}
...
static void RESAMPLE2()
{
	...
	temp =
	    ((s32) * (s16 *) (src + ((srcPtr + 0) ^ 1)) *
	     ((s32) ((s16) lut[0])));
	accum = (s32) (temp >> 15);

	temp =
	    ((s32) * (s16 *) (src + ((srcPtr + 1) ^ 1)) *
	     ((s32) ((s16) lut[1])));
	accum += (s32) (temp >> 15);

	temp =
	    ((s32) * (s16 *) (src + ((srcPtr + 2) ^ 1)) *
	     ((s32) ((s16) lut[2])));
	accum += (s32) (temp >> 15);

	temp =
	    ((s32) * (s16 *) (src + ((srcPtr + 3) ^ 1)) *
	     ((s32) ((s16) lut[3])));
	accum += (s32) (temp >> 15);
...
{				// Needs accuracy verification...
	...
	*(u8 *) (BufferSpace + ((cnt + v1) ^ 3)) =
	    *(u8 *) (BufferSpace + ((cnt + v0) ^ 3));
    }
}
...
static void ENVMIXER2()
{
	...
	if (!isMKABI)
		...
		vec10 =
		    (s16) (((s32) vec10 * (u32) env[5]) >> 0x10) ^ v2[3];
		if (inst1 & 0x10) {
		    temp = buffs0[x ^ 1] + vec10;
		    if (temp > 32767)
			temp = 32767;
...
void (*ABI2[0x20]) () = {
SPNOOP, ADPCM2, CLEARBUFF2, UNKNOWN, ADDMIXER, RESAMPLE2, UNKNOWN,
	SEGMENT2, SETBUFF2, DUPLICATE2, DMEMMOVE2, LOADADPCM2, MIXER2,
	INTERLEAVE2, HILOGAIN, SETLOOP2, SPNOOP, INTERL2, ENVSETUP1,
	ENVMIXER2, LOADBUFF2, SAVEBUFF2, ENVSETUP2, SPNOOP, HILOGAIN,
	SPNOOP, DUPLICATE2, UNKNOWN, SPNOOP, SPNOOP, SPNOOP, SPNOOP};
