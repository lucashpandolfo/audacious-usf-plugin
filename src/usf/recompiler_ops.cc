...
{
#ifdef USEX64
    MoveX86regToMemory(AddressReg, x86_R15,
		       (uintptr_t) TLBLoadAddress - (uintptr_t) TLB_Map);
#else
    MoveX86regToVariable(AddressReg, TLBLoadAddress);
#endif
    TestX86RegToX86Reg(LookUpReg, LookUpReg);
    CompileExit(Section->CompilePC, &Section->RegWorking, TLBReadMiss, 0,
		(void (*) (uintptr_t)) JeLabel32);
}
			   ...
			   int32_t BranchType, uint32_t Link)
{
	...
	if ((Section->CompilePC & 0xFFC) != 0xFFC) {
	    ...
	    case BranchTypeRs:
		EffectDelaySlot =
		    DelaySlotEffectsCompare(Section->CompilePC, Opcode.rs,
					    0);
		break;
	    case BranchTypeRsRt:
		EffectDelaySlot =
	    ...
	    case BranchTypeCop1:
		{
		    ...
		    if (!r4300i_LW_VAddr
			(Section->CompilePC + 4, &Command.Hex)) {
			...
			if (Command.fmt == R4300i_COP1_S
			    && (Command.funct & 0x30) == 0x30) {
			    EffectDelaySlot = 1;
			}
			if (Command.fmt == R4300i_COP1_D
			    && (Command.funct & 0x30) == 0x30) {
			    EffectDelaySlot = 1;
			}
		    }
	...
	Section->Jump.TargetPC =
	    Section->CompilePC + ((short) Opcode.offset << 2) + 4;
	...
	if (EffectDelaySlot) {
		...
		if (Section->Jump.LinkLocation != NULL) {
		    SetJump32((uint32_t *) Section->Jump.LinkLocation,
			      (uint32_t *) RecompPos);
		    Section->Jump.LinkLocation = NULL;
		    if (Section->Jump.LinkLocation2 != NULL) {
			SetJump32((uint32_t *) Section->Jump.LinkLocation2,
				  (uint32_t *) RecompPos);
			Section->Jump.LinkLocation2 = NULL;
		    }
		    Section->Jump.FallThrough = 1;
		    ...
		    SetJump32((uint32_t *) Section->Cont.LinkLocation,
			      (uint32_t *) RecompPos);
		    Section->Cont.LinkLocation = NULL;
		    if (Section->Cont.LinkLocation2 != NULL) {
			SetJump32((uint32_t *) Section->Cont.LinkLocation2,
				  (uint32_t *) RecompPos);
			Section->Cont.LinkLocation2 = NULL;
		    }
		    Section->Cont.FallThrough = 1;
	    ...
	    memcpy(&RegBeforeDelay, &Section->RegWorking,
		   sizeof(REG_INFO));
	}
	...
	if (EffectDelaySlot) {
	    JUMP_INFO *FallInfo =
		Section->Jump.FallThrough ? &Section->Jump : &Section->
		Cont;
	    JUMP_INFO *JumpInfo =
		Section->Jump.FallThrough ? &Section->Cont : &Section->
		Jump;
		...
		memcpy(&FallInfo->RegSet, &Section->RegWorking,
		       sizeof(REG_INFO));
		...
		if (!JumpInfo->DoneDelaySlot) {
			...
			SetJump32((uint32_t *) JumpInfo->LinkLocation,
				  (uint32_t *) RecompPos);
			JumpInfo->LinkLocation = NULL;
			if (JumpInfo->LinkLocation2 != NULL) {
			    SetJump32((uint32_t *) JumpInfo->LinkLocation2,
				      (uint32_t *) RecompPos);
			    JumpInfo->LinkLocation2 = NULL;
			}
			JumpInfo->FallThrough = 1;
			NextInstruction = DO_DELAY_SLOT;
			memcpy(&Section->RegWorking, &RegBeforeDelay,
			       sizeof(REG_INFO));
			return;
		    }
		}
...
void Compile_R4300i_BranchLikely(BLOCK_SECTION * Section,
				 ...
				 uint32_t Link)
{
	...
	if (Section->Cont.FallThrough) {
		...
		CompileExit(Section->CompilePC + 8, &Section->RegWorking,
			    Normal, 1, NULL);
		GenerateSectionLinkage(Section);
		NextInstruction = END_BLOCK;
	    } else {
...
void BNE_Compare(BLOCK_SECTION * Section)
{
	...
	if (IsConst(Opcode.rs) && IsConst(Opcode.rt)) {
		...
		CompX86RegToX86Reg(Is32Bit(Opcode.rs) ?
				   Map_TempReg(Section, x86_Any, Opcode.rs,
					       1) : MipsRegHi(Opcode.rs),
				   Is32Bit(Opcode.
					   rt) ? Map_TempReg(Section,
							     x86_Any,
							     Opcode.rt,
							     1) :
				   MipsRegHi(Opcode.rt)
		...
		CompX86RegToX86Reg(MipsRegLo(Opcode.rs),
				   MipsRegLo(Opcode.rt));
		if (Section->Cont.FallThrough) {
		    JneLabel32(0);
	    ...
	    uint32_t MappedReg =
		IsConst(Opcode.rt) ? Opcode.rs : Opcode.rt;
		...
		if (Is32Bit(ConstReg) || Is32Bit(MappedReg)) {
			...
			CompConstToX86reg(Map_TempReg
					  (Section, x86_Any, MappedReg, 1),
					  MipsRegHi(ConstReg));
		    } else {
			CompConstToX86reg(MipsRegHi(MappedReg),
					  (int32_t) MipsRegLo(ConstReg) >>
					  31);
		    }
		} else {
		    CompConstToX86reg(MipsRegHi(MappedReg),
		...
		if (MipsRegLo(ConstReg) == 0) {
		    OrX86RegToX86Reg(MipsRegLo(MappedReg),
				     MipsRegLo(MappedReg));
		} else {
		    CompConstToX86reg(MipsRegLo(MappedReg),
				      MipsRegLo(ConstReg));
		}
		if (Section->Cont.FallThrough) {
		    JneLabel32(0);
	...
	if (IsConst(KnownReg)) {
	    if (Is64Bit(KnownReg)) {
		CompConstToVariable(MipsRegHi(KnownReg),
				    &GPR[UnknownReg].W[1]);
	    } else if (IsSigned(KnownReg)) {
		CompConstToVariable(((int32_t) MipsRegLo(KnownReg) >> 31),
				    &GPR[UnknownReg].W[1]);
	    } else {
	    ...
	    if (Is64Bit(KnownReg)) {
		CompX86regToVariable(MipsRegHi(KnownReg),
				     &GPR[UnknownReg].W[1]);
	    } else if (IsSigned(KnownReg)) {
		ProtectGPR(Section, KnownReg);
		CompX86regToVariable(Map_TempReg
				     (Section, x86_Any, KnownReg, 1),
				     &GPR[UnknownReg].W[1]);
	    } else {
		CompConstToVariable(0, &GPR[UnknownReg].W[1]);
	    }
	...
	if (IsConst(KnownReg)) {
	    CompConstToVariable(MipsRegLo(KnownReg),
				&GPR[UnknownReg].W[0]);
	} else {
	    CompX86regToVariable(MipsRegLo(KnownReg),
				 &GPR[UnknownReg].W[0]);
	}
	if (Section->Cont.FallThrough) {
	    JneLabel32(0);
...
void BEQ_Compare(BLOCK_SECTION * Section)
{
	...
	CompX86regToVariable(Map_TempReg(Section, x86Reg, Opcode.rs, 0),
			     &GPR[Opcode.rt].W[0]);
	if (Section->Cont.FallThrough) {
	    JeLabel32(0);
	    Section->Jump.LinkLocation = RecompPos - 4;
...
void Compile_R4300i_J(BLOCK_SECTION * Section)
{
    if (NextInstruction == NORMAL) {
	Section->Jump.TargetPC =
	    (Section->CompilePC & 0xF0000000) + (Opcode.target << 2);;
	Section->Jump.FallThrough = 1;
	Section->Jump.LinkLocation = NULL;
	Section->Jump.LinkLocation2 = NULL;
	NextInstruction = DO_DELAY_SLOT;
...
void Compile_R4300i_JAL(BLOCK_SECTION * Section)
{
	...
	MipsRegState(31) = STATE_CONST_32;
	if ((Section->CompilePC & 0xFFC) == 0xFFC) {
	    MoveConstToVariable((Section->CompilePC & 0xF0000000) +
				(Opcode.target << 2), &JumpToLocation);
	...
	MoveConstToVariable((Section->CompilePC & 0xF0000000) +
			    (Opcode.target << 2), &PROGRAM_COUNTER);
...
void Compile_R4300i_ADDI(BLOCK_SECTION * Section)
{
	...
	MipsRegLo(Opcode.rt) =
	    MipsRegLo(Opcode.rs) + (short) Opcode.immediate;
...
void Compile_R4300i_SLTIU(BLOCK_SECTION * Section)
{
	...
	if (Is64Bit(Opcode.rs)) {
	    ...
	    Result =
		MipsRegLo(Opcode.rs) <
		((unsigned) ((short) Opcode.immediate)) ? 1 : 0;
	}
	UnMap_GPR(Section, Opcode.rt, 0);
	MipsRegState(Opcode.rt) = STATE_CONST_32;
	...
	if (Is64Bit(Opcode.rs)) {
	    ...
	    CompConstToX86reg(MipsRegHi(Opcode.rs),
			      ((short) Opcode.immediate >> 31));
	    JeLabel8(0);
	    Jump[0] = RecompPos - 1;
	    ...
	    *((uint8_t *) (Jump[0])) = (uint8_t) (RecompPos - Jump[0] - 1);
	    CompConstToX86reg(MipsRegLo(Opcode.rs),
			      (short) Opcode.immediate);
	    SetbVariable(&BranchCompare);
	...
	CompConstToVariable(((short) Opcode.immediate >> 31),
			    &GPR[Opcode.rs].W[1]);
	JneLabel8(0);
	Jump = RecompPos - 1;
	CompConstToVariable((short) Opcode.immediate,
			    &GPR[Opcode.rs].W[0]);
...
void Compile_R4300i_SLTI(BLOCK_SECTION * Section)
{
	...
	if (Is64Bit(Opcode.rs)) {
	    ...
	    Result =
		MipsRegLo_S(Opcode.rs) < (short) Opcode.immediate ? 1 : 0;
	}
	UnMap_GPR(Section, Opcode.rt, 0);
	MipsRegState(Opcode.rt) = STATE_CONST_32;
	MipsRegLo(Opcode.rt) = Result;
...
void Compile_R4300i_ORI(BLOCK_SECTION * Section)
{
	...
	if (Is64Bit(Opcode.rs)) {
	    ...
	    Map_GPR_32bit(Section, Opcode.rt, IsSigned(Opcode.rs),
			  Opcode.rs);
	}
...
void Compile_R4300i_XORI(BLOCK_SECTION * Section)
{
	...
	if (Opcode.immediate != 0) {
	    XorConstToX86Reg(MipsRegLo(Opcode.rt), Opcode.immediate);
	}
    }
}
...
void Compile_R4300i_LDL(BLOCK_SECTION * Section)
{
    ...
    if (Opcode.base != 0) {
	UnMap_GPR(Section, Opcode.base, 1);
    }
    if (Opcode.rt != 0) {
	UnMap_GPR(Section, Opcode.rt, 1);
    }
...
//      UnMap_TempReg(Section); //hax
	...
	uint32_t Address =
	    (MipsRegLo(Opcode.base) + (short) Opcode.offset) ^ 3;
	...
	if (Opcode.offset != 0) {
	    ...
	    LeaSourceAndOffset(TempReg1, MipsRegLo(Opcode.base),
			       (short) Opcode.offset);
	} else {
    ...
    MoveSxByteX86regPointerToX86reg(TempReg1, TempReg2,
				    MipsRegLo(Opcode.rt));
}
...
void Compile_R4300i_LH(BLOCK_SECTION * Section)
{
    ...
    if (IsConst(Opcode.base)) {
	uint32_t Address =
	    (MipsRegLo(Opcode.base) + (short) Opcode.offset) ^ 2;
	Map_GPR_32bit(Section, Opcode.rt, 1, 0);
    ...
    Map_GPR_32bit(Section, Opcode.rt, 1, -1);
    MoveSxHalfX86regPointerToX86reg(TempReg1, TempReg2,
				    MipsRegLo(Opcode.rt));
...
void Compile_R4300i_LW(BLOCK_SECTION * Section)
{
	...
	if (IsConst(Opcode.base)) {
	    uint32_t Address =
		MipsRegLo(Opcode.base) + (short) Opcode.offset;
	    Map_GPR_32bit(Section, Opcode.rt, 1, -1);
	...
	if (IsMapped(Opcode.base) && Opcode.offset == 0) {
		...
		if (Opcode.offset != 0) {
		    ...
		    TempReg1 =
			Map_TempReg(Section, x86_Any, Opcode.base, 0);
		}
	    } else {
		TempReg1 = Map_TempReg(Section, x86_Any, Opcode.base, 0);
		if (Opcode.immediate == 0) {
	...
	LOAD_FROM_TLB(TempReg2, TempReg2);
	CompileReadTLBMiss(Section, TempReg1, TempReg2);
	MoveX86regPointerToX86reg(TempReg1, TempReg2,
				  MipsRegLo(Opcode.rt));
    }
}
...
void Compile_R4300i_LBU(BLOCK_SECTION * Section)
{
    ...
    Map_GPR_32bit(Section, Opcode.rt, 0, -1);
    MoveZxByteX86regPointerToX86reg(TempReg1, TempReg2,
				    MipsRegLo(Opcode.rt));
...
void Compile_R4300i_LHU(BLOCK_SECTION * Section)
{
    ...
    XorConstToX86Reg(TempReg1, 2);
    Map_GPR_32bit(Section, Opcode.rt, 0, -1);
    MoveZxHalfX86regPointerToX86reg(TempReg1, TempReg2,
				    MipsRegLo(Opcode.rt));
...
void Compile_R4300i_SB(BLOCK_SECTION * Section)
{
	...
	if (IsConst(Opcode.rt)) {
	    ...
	    Compile_SB_Register(Map_TempReg
				(Section, x86_Any8Bit, Opcode.rt, 0),
				Address);
	}
	return;
    }
    ...
    XorConstToX86Reg(TempReg1 | x64_Reg, 3);
    if (IsConst(Opcode.rt)) {
	MoveConstByteToX86regPointer((uint8_t) MipsRegLo(Opcode.rt),
				     TempReg1, TempReg2);
    } else if (IsMapped(Opcode.rt) && Is8BitReg(MipsRegLo(Opcode.rt))) {
	MoveX86regByteToX86regPointer(MipsRegLo(Opcode.rt), TempReg1,
				      TempReg2);
    } else {
...
void Compile_R4300i_SH(BLOCK_SECTION * Section)
{
	...
	if (IsConst(Opcode.rt)) {
	    ...
	    Compile_SH_Register(Map_TempReg
				(Section, x86_Any, Opcode.rt, 0), Address);
	}
	return;
    }
    if (IsMapped(Opcode.rt)) {
    ...
    XorConstToX86Reg(TempReg1, 2);
    if (IsConst(Opcode.rt)) {
	MoveConstHalfToX86regPointer((uint16_t) MipsRegLo(Opcode.rt),
				     TempReg1, TempReg2);
    } else if (IsMapped(Opcode.rt)) {
	MoveX86regHalfToX86regPointer(MipsRegLo(Opcode.rt), TempReg1,
				      TempReg2);
    } else {
	MoveX86regHalfToX86regPointer(Map_TempReg
				      (Section, x86_Any, Opcode.rt, 0),
				      TempReg1, TempReg2);
    }
...
void Compile_R4300i_SW(BLOCK_SECTION * Section)
{
	...
	if (IsConst(Opcode.base)) {
		...
		Compile_SW_Register(Map_TempReg
				    (Section, x86_Any, Opcode.rt, 0),
				    Address);
	    }
	    return;
	...
	if (IsConst(Opcode.rt)) {
	    MoveConstToX86regPointer(MipsRegLo(Opcode.rt), TempReg1,
				     TempReg2);
	} else if (IsMapped(Opcode.rt)) {
	    MoveX86regToX86regPointer(MipsRegLo(Opcode.rt), TempReg1,
				      TempReg2);
	} else {
	    MoveX86regToX86regPointer(Map_TempReg
				      (Section, x86_Any, Opcode.rt, 0),
				      TempReg1, TempReg2);
	}
...
{
    if (!TranslateVaddr((uintptr_t *) & Address)) {
	DisplayError("Cache: Failed to translate: %X", Address);
	return;
    }
...
void Compile_R4300i_LD(BLOCK_SECTION * Section)
{
    ...
    MoveX86regPointerToX86regDisp8(TempReg1, TempReg2,
				   MipsRegLo(Opcode.rt), 4);
...
void Compile_R4300i_SD(BLOCK_SECTION * Section)
{
	...
	if (IsConst(Opcode.rt)) {
		...
		Compile_SW_Register(Map_TempReg
				    (Section, x86_Any, Opcode.rt, 1),
				    Address);
	    }
	    Compile_SW_Register(MipsRegLo(Opcode.rt), Address + 4);
	} else {
	    Compile_SW_Register(TempReg1 =
				Map_TempReg(Section, x86_Any, Opcode.rt,
					    1), Address);
	    Compile_SW_Register(Map_TempReg
				(Section, TempReg1, Opcode.rt, 0),
				Address + 4);
	}
	return;
    }
    ...
    if (IsConst(Opcode.rt)) {
	if (Is64Bit(Opcode.rt)) {
	    MoveConstToX86regPointer(MipsRegHi(Opcode.rt), TempReg1,
				     TempReg2);
	} else {
	    MoveConstToX86regPointer((MipsRegLo_S(Opcode.rt) >> 31),
				     TempReg1, TempReg2);
	}
	...
	if (Is64Bit(Opcode.rt)) {
	    MoveX86regToX86regPointer(MipsRegHi(Opcode.rt), TempReg1,
				      TempReg2);
	} else {
	    MoveX86regToX86regPointer(Map_TempReg
				      (Section, x86_Any, Opcode.rt, 1),
...
void Compile_R4300i_SPECIAL_JR(BLOCK_SECTION * Section)
{
	...
	if ((Section->CompilePC & 0xFFC) == 0xFFC) {
	    if (IsMapped(Opcode.rs)) {
		MoveX86regToVariable(MipsRegLo(Opcode.rs),
				     &JumpToLocation);
	    } else {
		MoveX86regToVariable(Map_TempReg
				     (Section, x86_Any, Opcode.rs, 0),
				     &JumpToLocation);
	    }
	    MoveConstToVariable(Section->CompilePC + 4, &PROGRAM_COUNTER);
	    if (BlockCycleCount != 0) {
	...
	if (DelaySlotEffectsCompare(Section->CompilePC, Opcode.rs, 0)) {
	    if (IsConst(Opcode.rs)) {
		MoveConstToVariable(MipsRegLo(Opcode.rs),
				    &PROGRAM_COUNTER);
	    } else if (IsMapped(Opcode.rs)) {
		MoveX86regToVariable(MipsRegLo(Opcode.rs),
				     &PROGRAM_COUNTER);
	...
	if (DelaySlotEffectsCompare(Section->CompilePC, Opcode.rs, 0)) {
	    ...
	    CompileExit((uint32_t) - 1, &Section->RegWorking, Normal, 1,
			NULL);
	} else {
	    if (IsConst(Opcode.rs)) {
		memcpy(&Section->Jump.RegSet, &Section->RegWorking,
		...
		CompileExit((uint32_t) - 1, &Section->RegWorking, Normal,
			    1, NULL);
	    }
	}
	NextInstruction = END_BLOCK;
...
{
    CompileExit(Section->CompilePC, &Section->RegWorking, DoSysCall, 1,
		NULL);
}
...
void Compile_R4300i_SPECIAL_MTLO(BLOCK_SECTION * Section)
{
	...
	if (Is64Bit(Opcode.rs)) {
	    MoveConstToVariable(MipsRegHi(Opcode.rs), &LO.UW[1]);
		   ...
		   && ((MipsRegLo(Opcode.rs) & 0x80000000) != 0)) {
	...
	MoveX86regToVariable(Map_TempReg(Section, x86reg, Opcode.rs, 0),
			     &LO.UW[0]);
    }
}
...
void Compile_R4300i_SPECIAL_DSRLV(BLOCK_SECTION * Section)
{
	...
	if (IsConst(Opcode.rt)) {
	    ...
	    MipsReg(Opcode.rd) =
		Is64Bit(Opcode.rt) ? MipsReg(Opcode.
					     rt) : (int64_t)
		MipsRegLo_S(Opcode.rt);
	    MipsReg(Opcode.rd) = MipsReg(Opcode.rd) >> Shift;
	    if ((MipsRegHi(Opcode.rd) == 0)
		&& (MipsRegLo(Opcode.rd) & 0x80000000) == 0) {
		MipsRegState(Opcode.rd) = STATE_CONST_32;
	    } else if ((MipsRegHi(Opcode.rd) == 0xFFFFFFFF)
		       && (MipsRegLo(Opcode.rd) & 0x80000000) != 0) {
		MipsRegState(Opcode.rd) = STATE_CONST_32;
	    } else {
...
void Compile_R4300i_SPECIAL_SUB(BLOCK_SECTION * Section)
{
	...
	if (IsConst(Opcode.rt)) {
	    ...
	    SubVariableFromX86reg(MipsRegLo(Opcode.rd),
				  &GPR[Opcode.rt].W[0]);
	}
    }
}
...
void Compile_R4300i_SPECIAL_AND(BLOCK_SECTION * Section)
{
	...
	if (IsConst(Opcode.rt) && IsConst(Opcode.rs)) {
		...
		MipsReg(Opcode.rd) =
		    (Is64Bit(Opcode.rt) ? MipsReg(Opcode.rt) : (int64_t)
		     MipsRegLo_S(Opcode.rt)) & (Is64Bit(Opcode.
							rs) ?
						MipsReg(Opcode.
							rs) : (int64_t)
						MipsRegLo_S(Opcode.rs));

		if (MipsRegLo_S(Opcode.rd) < 0
		    && MipsRegHi_S(Opcode.rd) == -1) {
		    MipsRegState(Opcode.rd) = STATE_CONST_32;
		} else if (MipsRegLo_S(Opcode.rd) >= 0
			   && MipsRegHi_S(Opcode.rd) == 0) {
		    MipsRegState(Opcode.rd) = STATE_CONST_32;
		} else {
		    MipsRegState(Opcode.rd) = STATE_CONST_64;
		}
		...
		MipsReg(Opcode.rd) =
		    MipsRegLo(Opcode.rt) & MipsReg(Opcode.rs);
		MipsRegState(Opcode.rd) = STATE_CONST_32;
	    }
	} else if (IsMapped(Opcode.rt) && IsMapped(Opcode.rs)) {
	    int32_t source1 =
		Opcode.rd == Opcode.rt ? Opcode.rt : Opcode.rs;
	    int32_t source2 =
		Opcode.rd == Opcode.rt ? Opcode.rs : Opcode.rt;

	    ProtectGPR(Section, source1);
	    ProtectGPR(Section, source2);
	    if (Is32Bit(source1) && Is32Bit(source2)) {
		int32_t Sign = (IsSigned(Opcode.rt)
				&& IsSigned(Opcode.rs)) ? 1 : 0;
		Map_GPR_32bit(Section, Opcode.rd, Sign, source1);
		AndX86RegToX86Reg(MipsRegLo(Opcode.rd),
				  MipsRegLo(source2));
		...
		if (IsUnsigned(Is32Bit(source1) ? source1 : source2)) {
		    ...
		    if (Is32Bit(source2)) {
			AndX86RegToX86Reg(MipsRegHi(Opcode.rd),
					  Map_TempReg(Section, x86_Any,
						      source2, 1));
		    } else {
			AndX86RegToX86Reg(MipsRegHi(Opcode.rd),
					  MipsRegHi(source2));
		...
		if (Is32Bit(MappedReg) && IsUnsigned(MappedReg)) {
		    ...
		    AndConstToX86Reg(MipsRegHi(Opcode.rd),
				     (uint32_t) (Value >> 32));
		    AndConstToX86Reg(MipsRegLo(Opcode.rd),
				     (uint32_t) Value);
		}
	    } else if (Is64Bit(MappedReg)) {
		uint32_t Value = MipsRegLo(ConstReg);
		if (Value != 0) {
		    Map_GPR_32bit(Section, Opcode.rd,
				  IsSigned(ConstReg) ? 1 : 0, MappedReg);
		    AndConstToX86Reg(MipsRegLo(Opcode.rd),
				     (uint32_t) Value);
		} else {
		    Map_GPR_32bit(Section, Opcode.rd,
				  IsSigned(ConstReg) ? 1 : 0, 0);
		}
	    } else {
		uint32_t Value = MipsRegLo(ConstReg);
		int32_t Sign = 0;
	...
	if (IsConst(KnownReg)) {
		...
		Map_GPR_32bit(Section, Opcode.rd, IsSigned(KnownReg),
			      UnknownReg);
		AndConstToX86Reg(MipsRegLo(Opcode.rd), (uint32_t) Value);
	    }
	} else {
		...
		if (Is64Bit(KnownReg)) {
		    ...
		    AndVariableToX86Reg(&GPR[UnknownReg].W[1],
					MipsRegHi(Opcode.rd));
		    AndVariableToX86Reg(&GPR[UnknownReg].W[0],
					MipsRegLo(Opcode.rd));
		} else {
		    Map_GPR_32bit(Section, Opcode.rd, IsSigned(KnownReg),
				  KnownReg);
...
void Compile_R4300i_SPECIAL_OR(BLOCK_SECTION * Section)
{
	...
	if (IsConst(Opcode.rt) && IsConst(Opcode.rs)) {
		...
		MipsRegLo(Opcode.rd) =
		    MipsRegLo(Opcode.rt) | MipsRegLo(Opcode.rs);
		MipsRegState(Opcode.rd) = STATE_CONST_32;
	    }
	} else if (IsMapped(Opcode.rt) && IsMapped(Opcode.rs)) {
	    int32_t source1 =
		...
		Map_GPR_64bit(Section, Opcode.rd, source1);
		if (Is64Bit(source2)) {
		    OrX86RegToX86Reg(MipsRegHi(Opcode.rd),
				     MipsRegHi(source2));
		} else {
		    OrX86RegToX86Reg(MipsRegHi(Opcode.rd),
				     Map_TempReg(Section, x86_Any, source2,
						 1));
		}
	    } else {
		ProtectGPR(Section, source2);
		...
		if (Is64Bit(ConstReg)) {
		    ...
		    Value =
			IsSigned(ConstReg) ? MipsRegLo_S(ConstReg) :
			MipsRegLo(ConstReg);
		}
		Map_GPR_64bit(Section, Opcode.rd, MappedReg);
		if ((Value >> 32) != 0) {
		    OrConstToX86Reg((uint32_t) (Value >> 32),
				    MipsRegHi(Opcode.rd));
		}
		if ((uint32_t) Value != 0) {
		    OrConstToX86Reg((uint32_t) Value,
				    MipsRegLo(Opcode.rd));
		}
	    } else {
		...
		if (Value != 0) {
		    OrConstToX86Reg(Value, MipsRegLo(Opcode.rd));
		}
	    }
	}
    } else if (IsKnown(Opcode.rt) || IsKnown(Opcode.rs)) {
	...
	if (IsConst(KnownReg)) {
	    ...
	    Value =
		Is64Bit(KnownReg) ? MipsReg(KnownReg) :
		MipsRegLo_S(KnownReg);
	    Map_GPR_64bit(Section, Opcode.rd, UnknownReg);
	    if ((Value >> 32) != 0) {
		OrConstToX86Reg((uint32_t) (Value >> 32),
				MipsRegHi(Opcode.rd));
	    ...
	    Map_GPR_64bit(Section, Opcode.rd, KnownReg);
	    OrVariableToX86Reg(&GPR[UnknownReg].W[1],
			       MipsRegHi(Opcode.rd));
	    OrVariableToX86Reg(&GPR[UnknownReg].W[0],
			       MipsRegLo(Opcode.rd));
	}
    } else {
	Map_GPR_64bit(Section, Opcode.rd, Opcode.rt);
...
void Compile_R4300i_SPECIAL_XOR(BLOCK_SECTION * Section)
{
	...
	if (IsConst(Opcode.rt) && IsConst(Opcode.rs)) {
		...
		Map_GPR_64bit(Section, Opcode.rd, source1);
		if (Is64Bit(source2)) {
		    XorX86RegToX86Reg(MipsRegHi(Opcode.rd),
				      MipsRegHi(source2));
		...
		XorX86RegToX86Reg(MipsRegLo(Opcode.rd),
				  MipsRegLo(source2));
	    } else {
		if (IsSigned(Opcode.rt) != IsSigned(Opcode.rs)) {
		    Map_GPR_32bit(Section, Opcode.rd, 1, source1);
		} else {
		    Map_GPR_32bit(Section, Opcode.rd, IsSigned(Opcode.rt),
				  source1);
		}
		XorX86RegToX86Reg(MipsRegLo(Opcode.rd),
				  MipsRegLo(source2));
		...
		ConstHi =
		    Is32Bit(ConstReg) ? (uint32_t) (MipsRegLo_S(ConstReg)
						    >> 31) :
		    MipsRegHi(ConstReg);
		ConstLo = MipsRegLo(ConstReg);
		Map_GPR_64bit(Section, Opcode.rd, MappedReg);
		if (ConstHi != 0) {
		    XorConstToX86Reg(MipsRegHi(Opcode.rd), ConstHi);
		}
		if (ConstLo != 0) {
		    XorConstToX86Reg(MipsRegLo(Opcode.rd), ConstLo);
		}
	    } else {
		int32_t Value = MipsRegLo(ConstReg);
		if (IsSigned(Opcode.rt) != IsSigned(Opcode.rs)) {
		    ...
		    Map_GPR_32bit(Section, Opcode.rd,
				  IsSigned(MappedReg) ? 1 : 0, MappedReg);
		}
		if (Value != 0) {
		    XorConstToX86Reg(MipsRegLo(Opcode.rd), Value);
		}
	    }
	}
    } else if (IsKnown(Opcode.rt) || IsKnown(Opcode.rs)) {
	...
	if (IsConst(KnownReg)) {
	    ...
	    Map_GPR_64bit(Section, Opcode.rd, UnknownReg);
	    if ((Value >> 32) != 0) {
		XorConstToX86Reg(MipsRegHi(Opcode.rd),
				 (uint32_t) (Value >> 32));
	    }
	    if ((uint32_t) Value != 0) {
	    ...
	    Map_GPR_64bit(Section, Opcode.rd, KnownReg);
	    XorVariableToX86reg(&GPR[UnknownReg].W[1],
				MipsRegHi(Opcode.rd));
	    XorVariableToX86reg(&GPR[UnknownReg].W[0],
				MipsRegLo(Opcode.rd));
	}
    } else {
	Map_GPR_64bit(Section, Opcode.rd, Opcode.rt);
...
void Compile_R4300i_SPECIAL_SLT(BLOCK_SECTION * Section)
{
	...
	if (IsConst(Opcode.rt) && IsConst(Opcode.rs)) {
		...
		CompX86RegToX86Reg(Is64Bit(Opcode.rs) ?
				   MipsRegHi(Opcode.
					     rs) : Map_TempReg(Section,
							       x86_Any,
							       Opcode.rs,
							       1),
				   Is64Bit(Opcode.rt) ? MipsRegHi(Opcode.
								  rt) :
				   Map_TempReg(Section, x86_Any, Opcode.rt,
					       1)
		    );
		...
		*((uint8_t *) (Jump[1])) =
		...
		if (MipsRegLo(Opcode.rd) > x86_EDX) {
		    SetlVariable(&BranchCompare);
		    MoveVariableToX86reg(&BranchCompare,
					 MipsRegLo(Opcode.rd));
		} else {
	    ...
	    uint32_t MappedReg =
		IsConst(Opcode.rs) ? Opcode.rt : Opcode.rs;

	    ProtectGPR(Section, MappedReg);
	    if (Is64Bit(Opcode.rt) || Is64Bit(Opcode.rs)) {
		uint8_t *Jump[2];

		CompConstToX86reg(Is64Bit(MappedReg) ? MipsRegHi(MappedReg)
				  : Map_TempReg(Section, x86_Any,
						MappedReg, 1),
				  Is64Bit(ConstReg) ? MipsRegHi(ConstReg)
				  : (MipsRegLo_S(ConstReg) >> 31)
		    );
		JeLabel8(0);
		Jump[0] = RecompPos - 1;
...
void Compile_R4300i_SPECIAL_SLTU(BLOCK_SECTION * Section)
{
	...
	if (IsConst(Opcode.rt) && IsConst(Opcode.rs)) {
		...
		*((uint8_t *) (Jump[1])) =
	    ...
	    if (Is64Bit(Opcode.rt) || Is64Bit(Opcode.rs)) {
		uint32_t MappedRegHi, MappedRegLo, ConstHi, ConstLo,
		    MappedReg, ConstReg;
		uint8_t *Jump[2];
		...
		if (Is32Bit(MappedReg)) {
		    MappedRegHi =
			Map_TempReg(Section, x86_Any, MappedReg, 1);
		}
		...
		*((uint8_t *) (Jump[1])) =
		...
		uint32_t Const =
		    IsConst(Opcode.rs) ? MipsRegLo(Opcode.
						   rs) : MipsRegLo(Opcode.
								   rt);
		uint32_t MappedReg =
		    IsConst(Opcode.rt) ? Opcode.rs : Opcode.rt;
...
void Compile_R4300i_SPECIAL_DADD(BLOCK_SECTION * Section)
{
	...
	MipsReg(Opcode.rd) =
	    Is64Bit(Opcode.rs) ? MipsReg(Opcode.
					 rs) : (int64_t)
	    MipsRegLo_S(Opcode.rs) +
	    Is64Bit(Opcode.rt) ? MipsReg(Opcode.
					 rt) : (int64_t)
	...
	if (IsConst(Opcode.rt)) {
	    ...
	    AddVariableToX86reg(MipsRegLo(Opcode.rd),
				&GPR[Opcode.rt].W[0]);
	    AdcVariableToX86reg(MipsRegHi(Opcode.rd),
				&GPR[Opcode.rt].W[1]);
	}
    }
}
...
void Compile_R4300i_SPECIAL_DSUB(BLOCK_SECTION * Section)
{
	...
	if (IsConst(Opcode.rt)) {
	    ...
	    SubVariableFromX86reg(MipsRegLo(Opcode.rd),
				  &GPR[Opcode.rt].W[0]);
	    SbbVariableFromX86reg(MipsRegHi(Opcode.rd),
				  &GPR[Opcode.rt].W[1]);
	}
    }
}
...
void Compile_R4300i_SPECIAL_DSLL(BLOCK_SECTION * Section)
{
	...
	MipsReg(Opcode.rd) =
	    Is64Bit(Opcode.rt) ? MipsReg(Opcode.
					 rt) : (int64_t)
	    MipsRegLo_S(Opcode.rt) << Opcode.sa;
	if (MipsRegLo_S(Opcode.rd) < 0 && MipsRegHi_S(Opcode.rd) == -1) {
	    MipsRegState(Opcode.rd) = STATE_CONST_32;
	} else if (MipsRegLo_S(Opcode.rd) >= 0
		   && MipsRegHi_S(Opcode.rd) == 0) {
    ...
    Map_GPR_64bit(Section, Opcode.rd, Opcode.rt);
    ShiftLeftDoubleImmed(MipsRegHi(Opcode.rd), MipsRegLo(Opcode.rd),
			 (uint8_t) Opcode.sa);
    ShiftLeftSignImmed(MipsRegLo(Opcode.rd), (uint8_t) Opcode.sa);
}
...
void Compile_R4300i_SPECIAL_DSRL(BLOCK_SECTION * Section)
{
    ...
    Map_GPR_64bit(Section, Opcode.rd, Opcode.rt);
    ShiftRightDoubleImmed(MipsRegLo(Opcode.rd), MipsRegHi(Opcode.rd),
			  (uint8_t) Opcode.sa);
...
void Compile_R4300i_SPECIAL_DSRA(BLOCK_SECTION * Section)
{
	...
	MipsReg_S(Opcode.rd) =
	    Is64Bit(Opcode.rt) ? MipsReg_S(Opcode.
					   rt) : (int64_t)
	    MipsRegLo_S(Opcode.rt) >> Opcode.sa;
	if (MipsRegLo_S(Opcode.rd) < 0 && MipsRegHi_S(Opcode.rd) == -1) {
	    MipsRegState(Opcode.rd) = STATE_CONST_32;
...
void Compile_R4300i_SPECIAL_DSRL32(BLOCK_SECTION * Section)
{
	...
	MipsRegState(Opcode.rd) = STATE_CONST_32;
	MipsRegLo(Opcode.rd) =
	    (uint32_t) (MipsReg(Opcode.rt) >> (Opcode.sa + 32));
    } else if (IsMapped(Opcode.rt)) {
	ProtectGPR(Section, Opcode.rt);
	if (Is64Bit(Opcode.rt)) {
	    if (Opcode.rt == Opcode.rd) {
		...
		Map_GPR_32bit(Section, Opcode.rd, 0, -1);
		MoveX86RegToX86Reg(MipsRegHi(Opcode.rt),
				   MipsRegLo(Opcode.rd));
	    }
	    if ((uint8_t) Opcode.sa != 0) {
		ShiftRightUnsignImmed(MipsRegLo(Opcode.rd),
				      (uint8_t) Opcode.sa);
	    }
	} else {
	    Compile_R4300i_UnknownOpcode(Section);
...
void Compile_R4300i_SPECIAL_DSRA32(BLOCK_SECTION * Section)
{
	...
	MipsRegState(Opcode.rd) = STATE_CONST_32;
	MipsRegLo(Opcode.rd) =
	    (uint32_t) (MipsReg_S(Opcode.rt) >> (Opcode.sa + 32));
    } else if (IsMapped(Opcode.rt)) {
	ProtectGPR(Section, Opcode.rt);
	if (Is64Bit(Opcode.rt)) {
	    if (Opcode.rt == Opcode.rd) {
	    ...
	    if ((uint8_t) Opcode.sa != 0) {
		ShiftRightSignImmed(MipsRegLo(Opcode.rd),
				    (uint8_t) Opcode.sa);
	    }
	} else {
	    Compile_R4300i_UnknownOpcode(Section);
...
void Compile_R4300i_COP0_MT(BLOCK_SECTION * Section)
{
	...
	Popad();
	*(uint8_t *) (Jump) =
	    (uint8_t) (((uint8_t) (uintptr_t) (RecompPos)) -
		       (((uint8_t) (uintptr_t) (Jump)) + 1));
...
#ifdef USEX64
    Call_Direct((void (*)())WriteTLBEntry);
#else
    Push(x86_ECX);
    Call_Direct((void (*)())WriteTLBEntry);
